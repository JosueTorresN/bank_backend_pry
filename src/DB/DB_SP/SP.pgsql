...
-- CORRECCIÓN: El SP debe devolver el hash para la autenticación
...
RETURNS TABLE (
  user_id UUID,
  rol UUID,
  contrasena_hash TEXT -- ¡ESTA COLUMNA ES ESENCIAL!
)
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  SELECT 
    u.id, 
    u.rol, -- (Asegúrate que la columna se llame 'rol' o 'rol_id' en tu tabla 'usuario')
    u.contrasena_hash 
  FROM usuario u
